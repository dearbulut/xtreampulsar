...
import {
  ...
  Users, ChevronUp, ChevronDown, ChevronsUpDown, Zap, Package,
} from 'lucide-react';
import {
  useResellerUsers,
...
function QuickCreateModal({ onClose, credits }: { onClose: () => void; credits: number }) {
  ...
  const customInsufficient = credits < 1;
  ...
  const handleCreate = async () => {
    ...
    if (tab === 'package') {
      ...
      const data = await packageMutation.mutateAsync({
        username: u, password: p, packageId: selectedPkgId, notes: notes.trim() || undefined,
      });
      setResult(data);
    } else {
  ...
  const disabled = isPending || (tab === 'package' ? !selectedPkgId || pkgInsufficient : customInsufficient);
        ...
        {tab === 'custom' && (
          <>
            ...
            <p className="text-xs text-muted">
              Bu işlem <span className="text-primary font-medium">1 kredi</span> düşecek (mevcut: {credits})
            </p>
